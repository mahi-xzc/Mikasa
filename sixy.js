...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
    return async function (event, message) {
        ...
        async function onStart() {
            ...
            // =========== FASTEST MENTION LOGIC (NO SEARCH) ===========
            // This filters out "MENTION_0" type ghost tags
            ...
            for (const id of mentionIDs) {
                // Keep only numeric IDs, remove "MENTION_X"
                if (!id.startsWith("MENTION_") && !isNaN(id)) {
                    realMentions[id] = mentions[id];
                    hasRealTag = true;
                }
            }
            ...
            if (hasRealTag) {
                ...
                // Auto-convert reply to mention format for commands that rely on mentions
                event.mentions = { [event.messageReply.senderID]: "" };
            } 
            else {
                // If only ghost tags exist, clear them so commands don't get garbage data
                event.mentions = {};
            }
            // =========== END OF LOGIC ===========
            ...
            if (adminPrefix && body.startsWith(adminPrefix)) {
                if (!isAdminBot) {
                    return await message.reply("❌ The admin prefix is only available for bot admins.");
                }
            ...
            if (needRole > role) {
                if (!hideNotiMessage.needRoleToUseCmd) {
                    if (needRole == 1)
                        ...
                        return await message.reply(`✖️ This command is only for developers!`);
                }
                else {
                    return true;
                }
            ...
            if (needRole > role) {
                if (!hideNotiMessage.needRoleToUseCmdOnReply) {
                    if (needRole == 1)
                        ...
                        return await message.reply(`⚠ | This command is for developers only.`);
                }
                else {
                    return true;
                }
