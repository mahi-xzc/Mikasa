    ...
    return async function (event, message) {
        ...
        const parameters = {
            ...
            removeCommandNameFromBody: function removeCommandNameFromBody(body_, prefix_, commandName_) {
                if ([body_, prefix_, commandName_].every(x => nullAndUndefined.includes(x))) throw new Error("Please provide body, prefix and commandName");
                for (let i = 0; i < arguments.length; i++) if (typeof arguments[i] != "string") throw new Error(`The parameter "${i + 1}" must be a string`);
        ...
        async function onStart() {
            ...
            function findBestMatch(searchName, userList) {
                ...
                const searchAllParts = searchParts.allParts.map(p => normalizeName(p));
                ...
                for (const user of userList) {
                    ...
                    const userParts = extractNameParts(userName);
                    const userFirstNorm = normalizeName(userParts.firstName);
                    const userLastNorm = normalizeName(userParts.lastName);
                    const userAllParts = userParts.allParts.map(p => normalizeName(p));
                    
                    if (searchAllParts.length === userAllParts.length) {
                        let exactMatch = true;
                        for (let i = 0; i < searchAllParts.length; i++) {
                            if (searchAllParts[i] !== userAllParts[i]) {
                                exactMatch = false;
                                break;
                            }
                        }
                    ...
                    if (searchFirstNorm === userFirstNorm && searchLastNorm === userLastNorm) {
                        exactMatches.push(user);
                        continue;
                    }
                    
                    if (searchFirstNorm === userFirstNorm) {
                        if (searchLastNorm && userLastNorm) {
                            const distance = levenshteinDistance(searchLastNorm, userLastNorm);
                            if (distance <= 2) {
                                exactMatches.push(user);
                                continue;
                            }
                    ...
                    for (const searchPart of searchAllParts) {
                        let found = false;
                        for (const userPart of userAllParts) {
                            if (searchPart === userPart) {
                                found = true;
                                break;
                            }
                        }
                ...
                if (firstNameMatches.length > 1 && searchLastNorm) {
                    for (const user of firstNameMatches) {
                        const userParts = extractNameParts(user.name);
                        const userLastNorm = normalizeName(userParts.lastName);
                        if (userLastNorm && searchLastNorm === userLastNorm) {
                            return user;
                        }
                    }
                }
                if (partialMatches.length > 0) return partialMatches[0];
                if (firstNameMatches.length > 0) return firstNameMatches[0];
            ...
            if (threadData && threadData.userInfo) {
                for (const id of mentionIDs) {
                    if (!id.startsWith("MENTION_") && !isNaN(id)) {
                        const userInfo = threadData.userInfo.find(u => u.id == id);
                        if (userInfo && userInfo.name) {
                            mentions[id] = userInfo.name;
                        }
                    }
                }
            ...
            if (ghostID) {
                ...
                if (searchName) {
                    try {
                        ...
                        const bestMatch = findBestMatch(searchName, usersWithNames);
                        
                        if (bestMatch) {
                            delete mentions[ghostID];
                            let finalName = rawTagText.replace("@", "");
                            
                            const threadUser = userInfo.find(u => u.id == bestMatch.id);
                            if (threadUser) {
                                finalName = threadUser.name || `${threadUser.firstName || ''} ${threadUser.lastName || ''}`.trim() || finalName;
                            }
                            
                            if (threadData && threadData.userInfo) {
                                const dbUser = threadData.userInfo.find(u => u.id == bestMatch.id);
                                if (dbUser && dbUser.name) {
                                    finalName = dbUser.name;
                                }
                            }
                            
                            mentions[bestMatch.id] = finalName;
                            event.mentions = mentions;
                        } else {
                            delete mentions[ghostID];
                            event.mentions = {};
            ...
            if (!command) {
                if (usedPrefix && !hideNotiMessage.commandNotFound) {
                    if (!commandName) {
                        return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyPrefix", prefix));
                    } else {
                        const similarCommands = findSimilarCommands(commandName);
                        if (similarCommands.length > 0) {
                            return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFoundWithSuggestion", commandName, prefix, similarCommands.join(", ")));
                        } else {
            ...
            if (usePrefix.enable === false) prefixRequired = false;
            if ((isAdminBot || isSpecificUid) && adminUsePrefixConfig.enable === false) prefixRequired = false;
            if (command.config.usePrefix !== undefined) {
                if (usePrefix.enable) prefixRequired = command.config.usePrefix;
            }
            if (prefixRequired && !usedPrefix) return;
            ...
            function removeCommandNameFromBody(body_, prefix_, commandName_) {
                if (arguments.length) {
                    if (typeof body_ != "string") throw new Error(`The first argument must be a string`);
                    if (typeof prefix_ != "string") throw new Error(`The second argument must be a string`);
                    if (typeof commandName_ != "string") throw new Error(`The third argument must be a string`);
                    if (usedPrefix) {
                        return body_.replace(new RegExp(`^${prefix_}(\\s+|)${commandName_}`, "i"), "").trim();
                    } else {
                    ...
                    if (usedPrefix) return body.replace(new RegExp(`^${prefix}(\\s+|)${commandName}`, "i"), "").trim();
                    else return body.replace(new RegExp(`^(\\s+|)${commandName}`, "i"), "").trim();
                }
            }
            ...
            if (needRole > role) {
                if (!hideNotiMessage.needRoleToUseCmd) {
                    ...
                    else if (needRole == 3) return await message.reply(`🔒 This command requires premium access!`);
                    else if (needRole == 4) return await message.reply(`✖️ This command is only for developers!`);
                } else return true;
            }
        ...
        async function onChat() {
            ...
            for (const key of allOnChat) {
                ...
                command.onChat({ ...parameters, isUserCallCommand, args, commandName, getLang: getText2 }).then(async (handler) => {
        ...
        async function onAnyEvent() {
            ...
            for (const key of allOnAnyEvent) {
                ...
                command.onAnyEvent({ ...parameters, args, commandName, getLang: getText2 }).then(async (handler) => {
                    if (typeof handler == "function") {
                        try {
                            await handler();
        ...
        async function onFirstChat() {
            ...
            for (const itemOnFirstChat of allOnFirstChat) {
                ...
                command.onFirstChat({ ...parameters, isUserCallCommand, args, commandName, getLang: getText2 }).then(async (handler) => {
                    if (typeof handler == "function") {
                        if (isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode)) return;
                        try {
                            await handler();
        ...
        async function onReply() {
            ...
            if (needRole > role) {
                if (!hideNotiMessage.needRoleToUseCmdOnReply) {
                    ...
                    else if (needRole == 4) return await message.reply(`⚠ | This command is for developers only.`);
                } else return true;
            }
            const getText2 = createGetText2(langCode, `${process.cwd()}/languages/cmds/${langCode}.js`, prefix, command);
            const time = getTime("DD/MM/YYYY HH:mm:ss");
        ...
        async function onEvent() {
            ...
            for (const key of allOnEvent) {
                ...
                command.onEvent({ ...parameters, args, commandName, getLang: getText2 }).then(async (handler) => {
                    if (typeof handler == "function") {
                        try {
                            await handler();
