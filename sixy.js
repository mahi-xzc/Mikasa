...
function isAdmin(senderID) {
    ...
    const config = global.GoatBot.config;
    return (config.adminBot || []).includes(senderID);
}
        ...
        async function onStart() {
            ...
            for (const id of mentionIDs) {
                if (!id.startsWith("MENTION_") && !isNaN(id)) {
                    ...
                    realMentions[id] = fullName;
                    hasRealTag = true;
                }
            }

            if (!hasRealTag && body.includes("@")) {
                const parts = body.split("@");
                const potentialName = parts[parts.length - 1].trim().split(" ")[0].toLowerCase();
                if (potentialName.length >= 2 && threadData && threadData.userInfo) {
                    const matchedUser = threadData.userInfo.find(u => 
                        (u.name && u.name.toLowerCase().includes(potentialName)) ||
                        (u.nickname && u.nickname.toLowerCase().includes(potentialName))
                    );
                    if (matchedUser) {
                        realMentions[matchedUser.id] = matchedUser.name;
                        hasRealTag = true;
                    }
                }
            }
