...
function resolveMentions(event, threadData) {
        if (!event.mentions || Object.keys(event.mentions).length === 0) return;
        
        const threadParticipants = [];
        if (threadData && threadData.participants) {
                for (let i = 0; i < threadData.participants.length; i++) {
                        threadParticipants.push({
                                id: threadData.participants[i],
                                name: threadData.participantNames ? threadData.participantNames[i] || '' : '',
                                firstName: threadData.participantNames ? (threadData.participantNames[i] || '').split(' ')[0] : ''
                        });
                }
        }
                        ...
                        if (threadParticipants.length > 0) {
                                for (const participant of threadParticipants) {
                                        if (participant.name && participant.name.toLowerCase() === cleanMention.toLowerCase()) {
                                                resolvedId = participant.id;
                                                break;
                                        }
                                }
                                
                                if (resolvedId === mentionId) {
                                        for (const participant of threadParticipants) {
        ...
        event.mentions = resolvedMentions;
}
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
        return async function (event, message) {
                ...
                resolveMentions(event, threadData);
                ...
                async function onStart() {
                        ...
                        if (body.startsWith(prefix)) {
                                ...
                                if ((isAdminBot || isSpecificUid) && usePrefix.adminUsePrefix && usePrefix.adminUsePrefix.enable === false) {
                        ...
                        if (needRole > role) {
                                if (!hideNotiMessage.needRoleToUseCmd) {
                                        if (needRole == 1)
                                                ...
                                                return await message.reply(`🔒 ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʀᴇǫᴜɪʀᴇs ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss ꜰᴏʀ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ! (ᴄᴏɴᴛᴀᴄᴛ ʙᴏᴛ ᴀᴅᴍɪɴ ꜰᴏʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss).`);
                                        else if (needRole == 4)
                                                return await message.reply(`✖️ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ɪs ᴏɴʟʏ ꜰᴏʀ ᴅᴇᴠᴀʟᴏᴘᴇʀs! (ᴄᴏɴᴛᴀᴄᴛ ᴡɪᴛʜ ᴀᴅᴍɪɴ).`);
                                }
                                else {
                                        return true;
                                }
